...
const SubmitProduct: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Simulating image upload for now (local API doesn't have storage yet)
      if (image) {
        imageUrl = `https://via.placeholder.com/400x300?text=${encodeURIComponent(formData.name)}`
      }
      ...
      await api.post('/products', {
        ...
        estimated_testing_time: formData.estimatedTestingTime || null,
        status: 'pending'
      })
      ...
      alert(`Error submitting product: ${error.message || 'Server error'}. Please try again.`)
    } finally {
